...
| `/api/internal/cardapio-totem-detalhe` | `{ clienteId, cwOrderId }` | `{ cwDisplayId:int, cwStatus, total, externalOrderId }` (o `externalOrderId` é o que o admin usa em "Confirmar criado") |
...
|---|---|---|
| `ENVIANDO` | INSERT feito, chamada em curso | → `CRIADO` (201) · → `REJEITADO` (determinístico) · → `AMBIGUO` (sem resposta, timeout, **qualquer 5xx** do HUB ou do CW, 4xx inesperado, ou parado há 5 min — job/admin) |
...
- `ENVIANDO` com `tentadoEm < agora−5min` (processo reiniciado entre o INSERT e o UPDATE) → `AMBIGUO` (transição permitida; o job nunca envia nada).
