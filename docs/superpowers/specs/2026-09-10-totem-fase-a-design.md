...
| `/api/internal/cardapio-totem-reconciliar` | `{ clienteId, orderId, orderType, tentadoEm, timeoutMs }` | `{ encontrado:true, cwOrderId, cwDisplayId, cwStatus, listados:n }` ou `{ encontrado:false, candidatos:n, listados:n }` (`listados` = tamanho da lista antes do filtro) |
...
| `REJEITADO` | HUB ou CW recusou de forma **comprovadamente pré-criação** (`COTACAO_*`, `422 CW_RECUSOU`, `401`, `429 CW_RATE_LIMIT`, 4xx determinístico do HUB); **nada foi criado** | terminal; o cliente pode refazer com **nova** chave/`orderId` |
