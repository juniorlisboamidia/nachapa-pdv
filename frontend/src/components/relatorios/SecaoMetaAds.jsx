...
// Seção Meta Ads: 4 KPIs (com comparativo vs período anterior) + 2 gráficos dos
// últimos 6 meses (Investimento vs Receita, Evolução do ROAS). Lógica copiada
// VERBATIM da seção Meta Ads do Dashboard (DashboardCliente.jsx).
...
export default function SecaoMetaAds({ dados, carregando }) {
  const k = dados?.kpis || {}
  ...
  if (!conectado) {
    return (
      <SemConta
        titulo="Nenhuma conta de Meta Ads conectada"
        descricao="Assim que a agência vincular uma conta de anúncios do Meta a esta loja, seu relatório aparece aqui automaticamente."
      />
    )
  }
  ...
  return (
    <div style={{ opacity: carregando ? 0.55 : 1, transition: 'opacity .15s' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginBottom: 20 }}>
        ...
        <KpiCard label="Compras" valor={num(k.compras)} atual={k.compras} anterior={ant?.compras} formatAbs={num} modo="bom-sobe" />
