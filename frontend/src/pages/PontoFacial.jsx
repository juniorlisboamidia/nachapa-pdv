...
function ModalProgressoEnvio({ envio, onClose }) {
  ...
  else { pct = Math.max(6, Math.min(92, Math.round(((Date.now() - inicio) / 18000) * 100))); cor = '#ea580c' }
                ...
                {DIAS_ABREV.map((lbl, i) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ padding: '6px 12px', borderRadius: 999, border: '1px solid ' + (on ? '#ea580c' : 'var(--app-border, #d4d4d4)'), background: on ? '#ea580c' : 'transparent', color: on ? '#fff' : 'inherit', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}
                    >
                      {lbl}
                    </button>
                  )
                ...
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '84px 78px 1fr 1fr 58px', alignItems: 'center', gap: 8 }}>
                  ...
                  {d.folga ? (
                    ...
                    <>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 600, color: cruzaMeiaNoite(d) ? '#ea580c' : 'transparent' }}>+1 dia</span>
                    </>
                  )}
                </div>
              ))}
      ...
      {erro ? (
        ...
        <>
          ...
          {dados.jaLancadas > 0 && !dados.bonificacaoFechada && (
            <div className="alert" style={{ background: '#fff7ed', border: '1px solid #bfdbfe', borderRadius: 10, padding: '10px 12px', marginBottom: 12, fontSize: 13, color: '#1e40af' }}>
