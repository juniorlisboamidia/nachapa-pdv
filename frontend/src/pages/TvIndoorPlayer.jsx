...
//
// A logo vem do bootstrap do aparelho (`Empresa.logoDataUrl`); sem ela, a inicial do nome.
function Institucional({ loja, aparelho }) {
  const nome = (loja?.nome ?? '').trim()
  const inicial = (nome || aparelho?.nome || '?').trim().charAt(0).toUpperCase()
  return (
    <div className="tv-raiz tv-institucional">
      {loja?.logoDataUrl
        ? <img className="tv-logo" src={loja.logoDataUrl} alt="" />
        : <div className="tv-inicial" aria-hidden="true">{inicial}</div>}
