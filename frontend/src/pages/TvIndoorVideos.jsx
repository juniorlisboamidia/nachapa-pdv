...
export default function TvIndoorVideos() {
  ...
  async function enviarArquivo(id, arquivo, meta) {
    ...
    try {
      ...
      await api.put(`/tv-indoor/videos/${id}/arquivo`, arquivo, {
        headers: cabecalhos,
        onUploadProgress: (e) => {
          if (!e.total) return
  ...
  async function salvar(form) {
    ...
    try {
      ...
      let id = form.id
      if (id) await api.put(`/tv-indoor/videos/${id}`, corpo)
      else {
        const r = await api.post('/tv-indoor/videos', corpo)
        id = r.data?.video?.id
      }
      // O upload vem DEPOIS do cadastro, e é o passo que pode falhar sozinho. Quando falha
      // num vídeo novo, a linha fica lá marcada como "Falta o arquivo" — de propósito: o
      // gestor reabre e reenvia, em vez de redigitar nome e agenda.
      if (id && form.arquivo) await enviarArquivo(id, form.arquivo, form.meta)
      setEditando(null)
      await buscar()
